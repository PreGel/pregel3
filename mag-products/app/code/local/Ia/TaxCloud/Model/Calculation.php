...
class Ia_TaxCloud_Model_Calculation extends Mage_Tax_Model_Calculation
{
     ...
     */
    public function setIncludeProducts(boolean $include)
    {
        if($include)
     ...
     * @return  Ia_TaxCloud_Model_Calculation
     */
    public function setIncludeShipping(boolean $include)
    {
        if($include)
    ...
    protected function _getTaxCloudRate($request)
    {
        if($request->getRegionId()==0){
            ...
            $cache_key .= $basedOnAddress->getStreet1().'_'.$basedOnAddress->getPostcode().'_';
            ...
            //} else {
                ...
                $destination->setCity($basedOnAddress->getCity());
                ...
                if($shippingTotal==0 && $productsTotal==0){
                    ...
                    $rates = array(
                                'products'=>round((($taxAmounts['products']/$productsTotal)*100),2),
                                'shipping'=>round((($taxAmounts['shipping']/$shippingTotal)*100),2),
