...
;(function ($, window, document, undefined) {
    ...
    var init_mega_menu = function() {
        ...
        // makrmark mod - need this for split menu 
        $('.top-bar-section .left li.custom-dropdown.has-dropdown').hoverIntent({
            over: show_dropdown,
            out: hide_dropdown,
            timeout: 100
        });
         // makrmark mod - need this for top menu 
        $('.top-bar-section .more-links li.custom-dropdown.has-dropdown').hoverIntent({
            over: show_dropdown,
            out: hide_dropdown,
            timeout: 100
        });
    ...
    var OrbitSlider = function(el, settings) {
        ...
        self.orbit_height = function() {
            if ($(container).find('.full-screen').length != 0 &&
                ...
                // makrmark - mod- changed class to from item to item-orbit - trying to avoid name collision with isotype grid items
                $(container).find("li.item-orbit").each(function(){
        ...
        self.fix_orbit_height = function(resize) {
             ...
             //makrmark - mod - continually checking for resize ? setting it to fixed height for now
            //var orbitHeight = 267;
            if(resize) {
                ...
                $(container).find("li.item-orbit img").on("load", function () { // waits until Interchange has placed the image
                    if (!isNaN(parseFloat(orbitHeight)) && isFinite(orbitHeight)) {
                        ...
                        //mark - mod - continually checking for resize ? setting it to fixed height for now
                        $(document).resize();
                    }
                });
            }
        ...
        self.orbit_bullets = function() {
            var orbitBulletsContainer = $(container).find('.orbit-bullets-container'),
                bullets = $(container).find('.'+settings.bullets_container_class+' > li'),
                  // makrmark - mod- changed class to from item to item-orbit - trying to avoid name collision with isotype grid items
                slides = $(container).find('li.item-orbit');
            ...
            if ($('html').hasClass('touch')) {
                $(window).on('orientationchange resize', function(e) {
                    // if(document.documentElement.clientWidth / parseFloat($("body").css("font-size")) >= 40.063) {
                        /*makrmark*/
                        //if(document.documentElement.clientWidth / parseFloat($("body").css("font-size")) >= 64.063) {
                        if(document.documentElement.clientWidth / parseFloat($("body").css("font-size")) >= 80.063) {
